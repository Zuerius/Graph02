...
void dfsFunction(IntList* inGraph, int numVert){
   ...
   for(i=0; i<numVert; i++){
      if(color[i] == 'w'){
         DFS(inGraph, dTime, fTime, parent, color, &time, i);
      }
   }
   printf("\nV  dT  fT  P");
   for(i=0; i<numVert; i++){
...
void DFS(IntList* inGraph, int* dTime, int* fTime, int* parent, char* color, int* time, int currentNode){
   color[currentNode] = 'g';
   *time = *time+1;
   ...
   while ( temp != intNil){
      ...
      if(color[edgeDest-1] == 'w'){
         DFS(inGraph, dTime, fTime, parent, color, time, edgeDest-1);
      }
